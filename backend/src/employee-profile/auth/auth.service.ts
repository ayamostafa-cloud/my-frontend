...
export class AuthService {
 ...
 async login(dto: { employeeNumber: string; password: string }) {
  ...
  // 🔥 ASSIGN ROLE USING ENUM (NO NEED FOR SCHEMA)
  // ⭐ DETERMINE REAL ROLE USING YOUR ENUM
  let role: SystemRole;
  ...
  if (n.startsWith("HRADM")) {
    ...
    role = SystemRole.DEPARTMENT_EMPLOYEE; // default
  }

  const payload = {
    id: employee._id.toString(),
    username: employee.firstName + " " + employee.lastName,
    role: role, // enum becomes a clean string
  };
...
async register(dto: RegisterDto) {
  ...
  if (exists) throw new BadRequestException("Employee number already exists ❌");
  ...
  const newEmployee = new this.employeeModel({
    ...
    address: dto.address,
    systemRole: dto.role,       // 🔥 VERY IMPORTANT
